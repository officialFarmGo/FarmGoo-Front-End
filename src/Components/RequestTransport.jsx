...
const RequestTransport = ({ onClose }) => {
  ...
  const fetchEstimate = useCallback(
    async (pickup, destination, vehicleId) => {
      ...
      try {
        const res = await axios.post(
          `${BaseUrl}/delivery/estimateDeliveryPrice`,
          {
        ...
        if (data?.data) {
          ...
          setEstimateError(
            data?.message || "Unable to get a delivery estimate right now.",
          );
        }
      } catch (err) {
        const message =
          ...
          err?.message ||
          "Unable to get a delivery estimate right now.";
  ...
  const handleChange = (e) => {
    ...
    if (name === "vehicleType") {
      ...
      if (updated.AddressOrpickUpLocation && updated.Destination && value) {
        fetchEstimate(
          updated.AddressOrpickUpLocation,
          updated.Destination,
          value,
        );
      }
  ...
  const validate = () => {
    ...
    if (!form.productType.trim())
      newErrors.productType = "Produce type is required";
    if (!form.quantity) newErrors.quantity = "Quantity is required";
    if (!form.weight) newErrors.weight = "Weight unit is required";
    if (!form.AddressOrpickUpLocation.trim())
      newErrors.AddressOrpickUpLocation = "Pickup address is required";
    if (!form.landMarkToAddressForPickup.trim())
      newErrors.landMarkToAddressForPickup = "Landmark is required";
    if (!form.Destination.trim())
      newErrors.Destination = "Destination is required";
    if (!form.customersName.trim())
      newErrors.customersName = "Customer name is required";
    if (!form.customersPhoneNumber.trim())
      newErrors.customersPhoneNumber = "Phone number is required";
  ...
  const isFormReady = Boolean(
    ...
    form.weight &&
    form.AddressOrpickUpLocation.trim() &&
    form.landMarkToAddressForPickup.trim() &&
    form.Destination.trim() &&
    form.customersName.trim() &&
    /^\d{10,11}$/.test(form.customersPhoneNumber.replace(/\s/g, "")) &&
    form.vehicleType,
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${BaseUrl}/delivery/createDelivery/${form.vehicleType}`,
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        throw new Error(
          data.message ||
            "Could not create transport request. Please try again.",
        );
      }
    ...
    <>
      ...
      {errorToast && (
        <div className="rt-error-toast">
          ...
          <button
            type="button"
            className="rt-error-toast-close"
        ...
        <div className="rt-success-popup-overlay">
          <div className="rt-success-popup">
            ...
            <p className="rt-success-desc">
              Your transport request has been submitted. A driver will be
      ...
      <div className="rt-modal-overlay">
        <div className="rt-modal-container">
          <header className="rt-modal-header">
            ...
            <button type="button" className="rt-close-btn" onClick={onClose}>
              &times;
            </button>
          </header>
          ...
          <div className="rt-form-scrollable">
            ...
            <div className="rt-card">
              ...
              <div className="rt-form-row">
                <div className="rt-form-group">
                  ...
                  />
                  {errors.productType && (
                    <span className="rt-field-error">{errors.productType}</span>
                  )}
                </div>
                <div className="rt-form-row-nested">
                  <div className="rt-form-group">
                    ...
                    />
                    {errors.quantity && (
                      <span className="rt-field-error">{errors.quantity}</span>
                    )}
                  </div>
                  <div className="rt-form-group">
                    <label>Weight</label>
                    ...
                    {errors.weight && (
                      <span className="rt-field-error">{errors.weight}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="rt-card">
              ...
              <div className="rt-form-group">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={
                    errors.AddressOrpickUpLocation ? "rt-input-error" : ""
                  }
                />
                {errors.AddressOrpickUpLocation && (
              ...
              <div className="rt-form-group">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={
                    errors.landMarkToAddressForPickup ? "rt-input-error" : ""
                  }
                />
                {errors.landMarkToAddressForPickup && (
                  <span className="rt-field-error">
                    {errors.landMarkToAddressForPickup}
                  </span>
                )}
            ...
            <div className="rt-card">
              ...
              <div className="rt-form-group">
                ...
                />
                {errors.Destination && (
                  <span className="rt-field-error">{errors.Destination}</span>
                )}
              </div>
            </div>
              ...
              <div className="rt-form-row">
                ...
                <div className="rt-form-group">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className={
                      errors.customersPhoneNumber ? "rt-input-error" : ""
                    }
                  />
                  {errors.customersPhoneNumber && (
                    <span className="rt-field-error">
                      {errors.customersPhoneNumber}
                    </span>
                  )}
            ...
            <div className="rt-card">
              ...
              <div className="rt-form-group">
                ...
                {errors.vehicleType && (
                  <span className="rt-field-error">{errors.vehicleType}</span>
                )}
              </div>
            </div>

            {estimateError && (
              <div className="rt-api-error-banner">{estimateError}</div>
            )}
                ...
                {estimate ? (
                  <>
                    <div className="rt-price-row">
                      ...
                      <p className="rt-price-value">
                        ₦{Number(estimate.deliveryFare).toLocaleString()}
                      </p>
                    </div>
                    <div className="rt-price-row">
                      <span>Service Fee</span>
                      <p className="rt-price-value">
                        ₦{Number(estimate.serviceFee).toLocaleString()}
                      </p>
                    </div>
                    <hr />
                  ...
                  <>
                    <div className="rt-price-row">
                      ...
                      <p className="rt-price-value">
                        {selectedVehicle
                          ? `₦${Number(selectedVehicle.baseFare).toLocaleString()}`
                          : "—"}
                      </p>
                    </div>
                    <div className="rt-price-row">
