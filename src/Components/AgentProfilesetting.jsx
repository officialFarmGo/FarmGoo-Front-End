...
const AgentProfileSettings = () => {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            console.warn("GET /profile returned 404. Fallback enabled.");
            return;
          }
        ...
        setErrors((prev) => ({
          ...prev,
          serverError: error.message || "Cannot reach the server.",
        }));
      } finally {
        setFetchLoading(false);
      }
  ...
  const handleFileChange = (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setErrors((prev) => ({
        ...prev,
        image: "File size exceeds 2MB. Please choose a smaller image.",
      }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ serverError: error.message || "Failed to update profile." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCancel = () => {
    ...
    setImageFile(null);
    if (initialData.profilePicture) setImagePreview(initialData.profilePicture);
    setErrors({});
  };

  // Layout Blueprint with added specific note field for UX mirroring picture reference blueprint
  const formFieldsConfig = [
    ...
    },
    {
      ...
      readOnly: true, // Swapped to True to match UI screenshot design restrictions
      note: (
        <span className="fg-input-note-label">
          Phone number cannot be changed here.{" "}
          <span
            className="fg-link-embedded"
            onClick={() =>
              alert("Redirecting to change number verification process...")
            }
          >
            Change phone number
          </span>
  ...
  return (
    <div className="fg-profile-settings-view">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Updated Adaptive Mobile Button Bar Layout */}
